...
void setup() {
  ...
  prefs.begin("ESPprefs", false);
  timerDelay = prefs.getInt("timerdelay", 10000);
  if (timerDelay<1) {
    timerDelay = 10;
    prefs.putInt("timerdelay", 1000);
  }
...
#ifdef WIFI
  host = prefs.getString("hostname", host);
  log::toAll("hostname: " + host);
  bool doubleReset = prefs.getBool("DRD", false);
